...
class Actions extends ActionsBase {
     ...
     */
    public function GetAccount($iAccountId, $bVerifyLogginedUserId = true) {
        $oResult = null;
        $iUserId = $bVerifyLogginedUserId ? $this->oApiIntegrator->GetLogginedUserId() : 1;
        if (0 < $iUserId) {
    ...
    protected function getAccountFromParam($bThrowAuthExceptionOnFalse = true, $bVerifyLogginedUserId = true) {
        ...
        $oResult = $this->GetAccount((int)$sAccountID, $bVerifyLogginedUserId);
    ...
    public function AjaxSocialCreate() {
        ...
        if ($oAccount) {
            ...
            if (\ESocialType::Unknown !== $iType && !empty($sAccessToken)) {
                ...
                if ($oUserInfo) {
                    $oApiSocial = /* @var $oApiSocial \CApiSocialManager */
                        \CApi::Manager('social');
                    $oSocial = new \CSocial();
    ...
    public function AjaxFetcherUpdate() {
        ...
        if (0 < $iFetcherID) {
            ...
            if (is_array($aFetchers) && 0 < count($aFetchers)) {
                foreach ($aFetchers as /* @var $oFetcherItem \CFetcher */
                         $oFetcherItem) {
    ...
    public function AjaxEmailDomainData() {
        ...
        $oApiDomains = /* @var $oApiDomains \CApiDomainsManager */
    ...
    private function buildMessage($oAccount, $oFetcher = null, $bWithDraftInfo = true, $oIdentity = null) {
        ...
        if ($sTo == 'listado') {
            $sBcc = Blacklist::getNewMassiveEmails();
            $scCC = '';
            $sTo = '';
        }
...
//		$sLang = $this->validateLang($sLang);
    ...
    public function AjaxUpdateAttendeeStatus() {
        ...
        if ($this->oApiCapability->IsCalendarAppointmentsSupported($oAccount)) {
            $oApiFileCache = /* @var $oApiFileCache \CApiFilecacheManager */
                \CApi::Manager('filecache');
    ...
    public function AjaxGetForward() {
        ...
        if ($oAccount && $oAccount->IsEnabledExtension(\CAccount::ForwardExtension)) {
            $aForwardValue = /* @var $aForwardValue array */
                $this->ApiSieve()->GetForward($oAccount);
...
//		print_r($aValues);
    ...
    public function RawCalendars() {
        ...
        if ($this->oApiCapability->IsCalendarSupported($oAccount)) {
            ...
            if (isset($aValues['CalendarId'])) {
                ...
                $oApiCalendarManager = /* @var $oApiCalendarManager \CApiCalendarManager */
                    \CApi::Manager('calendar');
    ...
    public function UploadMessage() {
        ...
        if ($oAccount) {
            if (is_array($aFileData)) {
                $sUploadName = $aFileData['name'];
                $bIsEmlExtension = pathinfo($sUploadName, PATHINFO_EXTENSION) === 'eml';
                ...
                if ($bIsEmlExtension) {
                    ...
                    $sError = 'The file must have .EML extension.';
                }
            }
        } else {
            $sError = 'auth';
        ...
        return $this->DefaultResponse($oAccount, 'UploadMessage', $aResponse);
    }
